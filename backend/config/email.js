...
const sendBrevoEmail = async (mailOptions) => {
  ...
  if (!apiKey.startsWith('xkeysib-')) {
    console.error('❌ Invalid BREVO_API_KEY format. Should start with "xkeysib-"');
  ...
  try {
    ...
    const response = await fetch('https://api.brevo.com/v3/smtp/email', {
      ...
      body: JSON.stringify({
        ...
        textContent: mailOptions.text || 'Your order has been confirmed.',
        htmlContent: mailOptions.html || '<h1>Your order has been confirmed!</h1>',
      }),
    });
...
export const sendOrderConfirmation = async (order, userEmail) => {
  try {
    ...
    console.log(`📧 Preparing order confirmation for: ${sanitizedEmail}`);
...
Scentify
    ...
    if (result) {
      ...
      console.error(`❌ Failed to send confirmation email to ${sanitizedEmail}`);
    }
...
export const sendAdminNotification = async (order) => {
  try {
    ...
    const mailOptions = {
      ...
      text: `New order #${orderIdShort} placed! Total: Rs. ${Number(order.totalPrice).toFixed(0)}`,
      html: adminHtml,
    };
